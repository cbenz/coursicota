...
export function listProductsByFrequency(): ProductFrequencyRow[] {
  ...
  const rows = db
    .prepare(
      `
      SELECT
        COALESCE(product_id, url, name) AS key,
        product_id,
        url,
        name,
        packaging,
        COUNT(DISTINCT order_id) AS occurrences,
        ROUND(AVG(COALESCE(quantity, 1))) AS suggested_quantity
      FROM order_products
      GROUP BY key, product_id, url, name, packaging
