<script lang="ts">
  ...
  function toggleRowSelection(product: OrderProductRow, nextChecked: boolean): void {
    ...
    {#if selectedCount > 0}
      <div class="w-full rounded-md border p-4 lg:w-1/2">
        ...
        >
          ...
          <Button type="submit" size="sm" class="sm:col-span-2 sm:justify-self-start">
            Add to list
          </Button>
        </form>
      </div>
      ...
      <Table.Root>
          ...
          {#each headerGroups as headerGroup (headerGroup.id)}
            <Table.Row>
              ...
              {#each headerGroup.headers as header (header.id)}
                <Table.Head
                  class={header.column.id === "totalPrice" ? "text-right" : undefined}
                >
        ...
        <Table.Body>
          {#each rows as row (row.id)}
            ...
            <Table.Row class={selectedKeys.includes(rowKey) ? "bg-muted/40" : undefined}>
              <Table.Cell
                class="text-center"
                style={`width: ${selectionColumnWidth}px; min-width: ${selectionColumnWidth}px; max-width: ${selectionColumnWidth}px;`}
              >
              ...
              {#each row.getVisibleCells() as cell (cell.id)}
                <Table.Cell class={cell.column.id === "totalPrice" ? "text-right" : undefined}>
                  <FlexRender
